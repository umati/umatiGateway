...
namespace umatiGateway.Core.Mqtt.CustomEncoding
{
    ...
    public class CustomEncodingManager
    {
        ...
        {
            managedCustomEncodings.Add(new ManagedCustomEncoding(new GMSResultDataTypeEncoding()));
            managedCustomEncodings.Add(new ManagedCustomEncoding(new ProcessingCategoryDataTypeEncoding()));
        }
        public ManagedCustomEncoding? GetManagedCustomEncodingByName(string name)
        {
            foreach (ManagedCustomEncoding managedCustomEncoding in managedCustomEncodings)
            {
            ...
            if (customEncodingNodes != null)
            {
                foreach (XmlNode customEncodingNode in customEncodingNodes)
                {
                    if (customEncodingNode.Attributes != null)
                    {
                        ...
                        if (name != null)
                        {
                            ManagedCustomEncoding? managedCustomEncoding = GetManagedCustomEncodingByName(name);
